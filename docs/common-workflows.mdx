...
Use this page after choosing the result your application needs. Each workflow identifies the starting input, product sequence, final result, and point where document data leaves the local machine.

| Application goal | Recommended path | Final result |
| --- | --- | --- |
| Validate an upload before processing | DeckProbe → application policy | Accepted, rejected, or routed document |
| Build a searchable document corpus | DeckOps Parse | Markdown or format-specific structured content |
| Add document previews to a review UI | DeckRender | Page images, PDF, or supported video |
| Personalize an existing presentation | DeckUse | Updated editable PPTX |
| Generate presentations from application data | DeckHTML | PPTX or PNG plus conversion diagnostics |
| Validate, parse, and preview one upload | DeckProbe → DeckOps → DeckRender | Policy decision, content, and visual artifacts |

## Gate document uploads before cloud processing

**Start with:** an untrusted PDF, Office, or modern iWork file received by your application.

1. Run DeckProbe locally for identity, encryption, macro, structure, count, or asset targets.
2. Apply your application's acceptance and routing policy to the Schema v2 evidence.
3. Send only accepted files to DeckOps or DeckRender when downstream cloud processing is required.

**Result:** an evidence-backed accept, reject, or routing decision. Document bytes stay local during DeckProbe and leave the machine only when your application passes them to a cloud product.
...
**Start with:** a supported PDF, PPTX, DOCX, Keynote file, or HTML URL.
...
**Result:** Markdown for indexing and model input, or a format-specific structured response for layouts, notes, elements, and semantic roles where implemented.
...
**Start with:** a supported document, URL, or stdin stream.
...
**Result:** written artifacts plus a `RenderResult` containing route, page count, output files, byte sizes, duration, and fidelity caveats.
...
**Start with:** an editable `.pptx` that must remain the source of truth.
...
**Result:** another editable PPTX created locally. The current release does not add or delete elements and does not edit DOCX, XLSX, or Keynote.
...
**Start with:** HTML or SVG authored from templates, application data, or generated content.
...
**Result:** PPTX or PNG plus diagnostics that identify native, vector, raster, ignored, and unsupported mappings.
