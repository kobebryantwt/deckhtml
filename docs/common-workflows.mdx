...
<div className="workflow-meta"><span className="workflow-meta-label">Final result</span><span className="workflow-meta-value">Another editable PPTX created locally, plus a reproducible command and operation record. PPTX is the only implemented editing adapter.</span></div>

<Card title="Start with DeckUse" href="/deckuse/quick-start">
  Install from source, update a known PPTX through JSON, validate the workspace, and verify the exported file.
</Card>
