...
| Interfaces | Native CLI, npm CLI, Browser SDK, Web Worker SDK, and Node.js API |
| Authentication | None |

<Note>
  Version 2.3.1 fixes the JavaScript package contents and passes the Browser and package-completeness checks. If `npm view @deckflow/deckprobe version` still resolves to 2.3.0, use the native CLI or build the 2.3.1 package from source as shown on the SDK pages until the Registry catches up.
</Note>
