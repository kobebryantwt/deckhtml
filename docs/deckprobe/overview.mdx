...
DeckProbe answers targeted questions about an existing document without rendering it, running macros, or launching Microsoft Office, Apple iWork, or another desktop suite. The native CLI reads lazily from disk; the repository's JavaScript implementation runs the same Rust engine through WebAssembly in browsers and Node.js.
...
| Interfaces | Native CLI and npm CLI; Browser, Worker, and Node.js source APIs awaiting a corrected npm package |
| Authentication | None |
...
- Routing a document to a parser or renderer based on evidence instead of extension alone.
...
The default report is deterministic when telemetry is disabled. Add telemetry only when elapsed time is needed.
