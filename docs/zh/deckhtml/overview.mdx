...
title: DeckHTML
sidebarTitle: 概览
description: "把浏览器渲染后的 HTML 或 SVG 转为 PPTX 或 PNG，并明确记录可编辑映射与降级结果。"
---

DeckHTML 会在 Chromium 中加载 HTML 或 SVG、测量实际渲染布局，再生成 PPTX 或 PNG。生成 PPTX 时，它会把支持的文字、图片、表格、形状、SVG 几何和公式重建为可编辑或矢量对象；没有可靠 PowerPoint 映射时，则用栅格图保留外观，并在转换报告中记录这一决定。
