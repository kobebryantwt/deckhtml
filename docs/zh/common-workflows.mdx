...
确定应用需要的结果后，再通过本页选择实际工作流。每个工作流都会说明起始输入、产品组合、最终结果，以及文档数据在何处离开本机。

| 应用目标 | 推荐路径 | 最终结果 |
| --- | --- | --- |
| 处理前校验上传文件 | DeckProbe → 应用策略 | 接收、拒绝或路由文档 |
| 构建可检索的文档语料 | DeckOps Parse | Markdown 或格式专属结构化内容 |
| 为评审界面增加文档预览 | DeckRender | 页面图片、PDF 或支持的视频 |
| 个性化现有演示文稿 | DeckUse | 修改后仍可编辑的 PPTX |
| 从应用数据生成演示文稿 | DeckHTML | PPTX 或 PNG，以及转换诊断 |
| 校验、解析并预览同一上传文件 | DeckProbe → DeckOps → DeckRender | 策略判断、内容和视觉产物 |

## 在云端处理前校验文档上传

**起始输入：**应用接收到的不可信 PDF、Office 或现代 iWork 文件。

1. 在本地运行 DeckProbe，获取身份、加密、宏、结构、数量或资源 Target。
2. 根据 Schema v2 证据执行应用自己的接收和路由策略。
3. 需要下游云端处理时，只把已接收文件发送给 DeckOps 或 DeckRender。

**最终结果：**基于证据的接收、拒绝或路由判断。DeckProbe 阶段文档字节留在本地，只有应用把文件传给云端产品时才会离开本机。
...
**起始输入：**受支持的 PDF、PPTX、DOCX、Keynote 文件或 HTML URL。

应用需要 Markdown 或源格式结构而不是像素时，使用 DeckOps Parse。把 Task ID 与派生结果一起保存，让失败、重试和格式专属 Schema 保持可观测。

**最终结果：**用于索引和模型输入的 Markdown，或在已实现范围内包含布局、备注、元素和语义角色的格式专属结构化响应。
...
**最终结果：**写入的产物，以及包含路由、页数、输出文件、字节数、耗时和保真限制的 `RenderResult`。
...
**起始输入：**必须继续作为事实源的可编辑 `.pptx`。

使用 DeckUse 检查 Selector、替换或设置现有文本，并移动受支持的 Shape。一次输出包含多项修改时，使用持久化 Workspace。

**最终结果：**在本地生成的另一个可编辑 PPTX。当前版本不能新增或删除元素，也不能编辑 DOCX、XLSX 或 Keynote。
