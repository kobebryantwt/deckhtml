...
| 接入方式 | 原生与 npm CLI、Browser 与 Worker SDK、Node.js API、Agent Skill 和本地 MCP Server |
| 认证 | 不需要 |
...
<CardGroup cols={2}>
  ...
  <Card title="Agent 集成" href="/zh/deckprobe/agent-integrations/overview">
    通过 Agent Skill 使用 CLI，或通过本地 MCP Server 暴露有类型约束的工具。
  </Card>
</CardGroup>
...
- 需要把页面与元素内容保存为可复用 DeckIR，或生成 Markdown 视图时，使用 [DeckOps](/zh/deckops/overview)。
- 需要修改现有 PPTX 中支持的属性时，使用 [DeckUse](/zh/deckuse/overview)。

## 源码与支持

DeckProbe 完全开源。可以前往 [DeckProbe 开源项目](https://github.com/deckflow/deckprobe)查看实现与版本发布，或提交 Issue。
