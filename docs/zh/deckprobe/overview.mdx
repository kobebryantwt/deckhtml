...
DeckProbe 用于回答关于现有文档的指定问题，不会渲染文档、运行宏，也无需启动 Microsoft Office、Apple iWork 或其他桌面套件。原生 CLI 会从磁盘惰性读取；仓库中的 JavaScript 实现通过 WebAssembly 在浏览器和 Node.js 中运行同一套 Rust 引擎。
...
| 接入方式 | 原生 CLI 和 npm CLI；Browser、Worker、Node.js 源码 API 等待修正后的 npm 包 |
| 认证 | 不需要 |
...
- 根据证据而不是扩展名，把文档路由给 Parser 或 Renderer。
- 限制物理读取、解压、归档条目和时间预算。
...
关闭 Telemetry 时，默认报告是确定性的。只有需要耗时信息时才添加 Telemetry。
