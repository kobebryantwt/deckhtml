...
DeckOps 没有对外提供一个跨格式统一的 Document IR。高层 `parse()` 接口把支持的解析结果统一为 Markdown；底层解析任务则为每种格式保留不同的结构化契约。持久化或转换这些响应前，请先阅读[结构化解析结果](/zh/deckops/structured-results)。
...
<Note>
  不同 SDK 提供的接口有所不同。上面的底层任务流程是最直接的可安装路径；使用更高层 Parse 方法前，请先在[解析文档](/zh/deckops/parse)中确认可用性。
</Note>
...
<Warning>
  使用 `parse()` 和 `parseDetailed()` 时请配置 API Key 或用户 Token。需要游客模式时，请使用底层任务 API，并在创建后调用 `tasks.start(task.id)`。Python SDK 没有提供这个游客启动方法。
</Warning>

## 继续阅读

<CardGroup cols={2}>
  <Card title="快速开始" href="/zh/deckops/quick-start">完成并验证一次完整的游客云端任务。</Card>
